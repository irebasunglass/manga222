...
class MangaReaderApp:
    ...
    def build_ui(self):
        ...
        ttk.Label(capture_frame, text="※ ボタンを押したら読みたい画面へ切り替え").pack(side="left", padx=10)
    ...
    def extract_text_from_image(self, image_path: str, auto_speak: bool = False):
        try:
            ...
            text = self.ocr.image_to_text(image_path)
            if not text.strip():
    ...
    def ocr_from_selection(self):
        self.set_status("範囲選択を開始します。ドラッグして範囲を選んでください。Escで中止できます。")
        self.root.iconify()
